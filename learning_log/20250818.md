...
## 람다식의 등장 배경
- 람다식이 나오기 전에는 Java에서 함수형 프로그래밍을 하기 위해 익명 클래스를 주로 사용(첫 번째 코드 블럭 예시)
- 하짐나 이상의 방식은 예제에서 볼 수 있듯이 코드가 길어지고, 불필요한 반복이 많아지며, 가독성이 떨어지는 문제가 있다.
  (call1() 유형의 코드를 5개 정의한다고 하면 runnable1 ... runnable 5)
...
`@FunctinalInterface` 애너테이션을 사용하면 컴파일 시에 단일 메서드인지 체크합니다.
- 그래서 '사용자 정의 함수형 인터페이스'를 작성할 때 레벨에 `FunctionalInterface`를 명시해줍니다.
...
public static void main(String[] args) {
    ...
    // 매개변수가 아예 없는 경우는 ()로 표시합니다. 매개변수가 하나면 소괄호 생략
...
5. Predicate - boolean test(T t); - 얘는 return 타입이 boolean으로 고정이라 call1~call4로 묶기
애매하긴 한데 굳이 따지자면 call4();입니다. return true / false 중 하나니까요.
...
4. Function<T, R>
- '하나의 매개변수'를 받아서, 연산을 수행한 후결과를 리턴함. 
...
public static void main(String[] args) {
    ...
    // and() 로 두 조건 결합
    Predicate<String> startsWithAndEndsWithX = startsWithA.and(endsWithX);

    System.out.println(startsWithAndEndsWithX.test("Alex"));
...
ctrl + shift + f 누르면 파일 전체에서 검색이 가능하다.
