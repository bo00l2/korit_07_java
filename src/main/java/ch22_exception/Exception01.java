...
/*
    ...
    그럼 Exception01에서 알 수 있듯이, 다수의 예외의 경우에는 이미 개발자들이 정의를 해놓았다.
    예를 들어 List를 하나 정의했고, 거기에 element들을 집어넣었다고 가정하자.
...
//        int b = 0;
//        try {
//            int result = a / b;     // ArithmeticException 예외 발생 지정 -> 37번 라인이 아니라 catch문으로 넘어간다.
...
//        }
        ...
        try {
            ...
            throw new RuntimeException(e);      // 사실 저희가 보던 시뻘건게 이 오류 메시지였다.
        } finally {
            System.out.println("프로그램 정상 종료");
        }

        /*
            이상의 코드 라인에서 중요한 점은 catch문이 복수로 나올 수 있다는 점이다.
            우리가 if문(leap year 관련)에 대해서 작성할 때
            조건의 순서를 배치하는 것이 매우 중요하다고 했다.
