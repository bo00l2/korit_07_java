...
public class CentralControl {
    // 필드 선언
    private Power[] deviceArray; // 업캐스팅
    ...
    public void addDevice(Power device){
        ...
        // 비어있는 element device 추가를 할 수 있어야 한다.
        ...
        deviceArray[emptyIndex] = device;       // private으로 적용해놔서 method를 경유했다.
        // public이면 Main에서 그냥 바로 위의 코드를 집어넣으면 된다.
...
/*
    ...
    private int checkEmpty(){ // 메서드인데 private를 썼다. Main에서 굳이
        // 몇 개나 더 전자제품을 추가할 수 있는지 알 필요가 없을 것 같아서 그럼
        // deviceArray라는 배열에서 비어있는 index를 추출해주기만 하면 된다.
        // i++니까 가장 빨리 만나게 되는 null값을 return 하게 될 것이다.
        ...
        return -1;
        /*
            Java의 index 넘버에는 음수값이 없기 때문에(python에서는 마이너스인덱스 개념이 잇는데 얘도
            시험에 자주 나온다.) 실패를 나타낼 때 -1 을 쓰는 경우가 많다.
            하지만 0과 너무 가까운 수이다보니 -100이든지, 아니면 return 값으로 나눌 수 없으만한
            음수값을 지정하는 경우도 있는데,
    ...
    /*
        현재 배열 내엥 각 객체들이 들어가있다.
        객체명.getClass().getSimpleName()을 활용하면 클래스 네임만 출력된다는 것을 확인할 수 잇는데,
    ...
    public void performSpecificMethod(){
        for(Power device : deviceArray){
            ...
            } else if(device instanceof Computer){      // 결과값은 위
            ...
            } else {        // 아직 instanceof 연산자를 통해서 조건문을 추가하지 못한 class alc
                            // 추후 update를 통해서 다른 기기들을 지원할 때는 } else if문을 추가하면 됨.
                System.out.println("아직 지원하지 않는 기기입니다.");
            }
        }
    }
    // 이상의 수업 과정들을 전부 거쳤을 때, Printer 클래스 정의 Power interface를 implement히거
    // 고유 메서드로 print() -> "프린ㅌ어가 인쇄를 합니다."로 출력될 수 있도록 정의하고,
    // Main에서 printer1을 centralControl에 추가, on() / off() 를 실행한 후
