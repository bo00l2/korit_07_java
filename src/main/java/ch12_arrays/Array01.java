...
//        System.out.println(i);          // 결과값 : 1
        ...
        // 현재 상황에서 배열 내부의 값을 확인할 수가 없다. 주소지만 출력됨
        // 배열 내부의 각 숫자들을 원소라고 한다.
        // 현재 배열 내부에는 다수의 데이터가 있기 때문에 콘솔창에서 전체 확인이 불가능
        // 즉, element들을 확인하기 위해서는 배열 내부로 직접 탐색하는 과정이 필요하다
        // 이때 필요한 개념이 index(주소지) 개념이다.
...
//        System.out.println(arr01[4]);
        ...
        // index만 빼고 나머지가 반복됨 -> 반복문을 사용
        // 반복문을 활용하여 arr01의 내부 element들을 콘솔에 출력
        // 1 2 3 4 5 로 출력
        for (int i=0; i<5; i++){            // 5가 하드코딩 되어있음 -> literal을 그대로 썼다.
            System.out.print(arr01[i] + " ");       // i가 어디에 적용되어야하는지 감안할 필요가 있다
        }
        System.out.println();
        ...
        // 근데 우리가 필드를 참조할 때 객체명.필드 형태로 참조했었다.
        // bank1.accountHolder와 같은 방식으로 참조 가능했다
        // 그리고 field를 직접 참조했다는 말은 length 필드는 public이라고 볼 수 있겠다.
        ...
        // 그래서 다른 자료형의 배열이더라도 기본적으로 element를 추출하는 절차는 동일하다.
    }
}
