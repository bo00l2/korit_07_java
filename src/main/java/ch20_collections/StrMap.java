...
public class StrMap {
    public static void main(String[] args) {
        ...
        // key 하나당 value는 하나입니다. 즉 키가 같다면 value는 가장 최근 걸로 덮어쓰기가 됩니다.
        ...
        koreanscores1.put("김일", 98.2);
        koreanscores1.put("김이", 87.2);
        koreanscores1.put("김삼", 46.1);
        koreanscores1.put("김사", 65.7);
        koreanscores1.put("김오", 4.5);
        koreanscores1.put("김육", 100.0);

        Map<String, Double[]> koreanscores2 = new HashMap<>();
        koreanscores2.put("김일", new Double[2]); // 이건 자료구조가 복잡하다.

        // 특정 key의 value 수정
        // #  1. 아까 말한 것처럼 key에는 value가 하나 밖에 없기 때문에 최근걸로 덮어쓰기 된다.
        System.out.println(koreanscores1);
        koreanscores1.put("김오", 4.6);
        System.out.println(koreanscores1);

        // # 2. .replace();
        koreanscores1.replace("김육", 789.2);
        System.out.println(koreanscores1);
        /*
...
//        List<Map.Entry<String, String>> entryList2 = strMap1.entrySet();
        ...
        // 이상까지의 과정에서 Map -> Set -> List 로 바꾸는 과정을 코드로 작성했는데
        // Map -> List로 바로 가는 게 가능한지 여부를 따질 필요가 있다.

        // .sort()도 가능하고 추출도 가능하겠다

        // Key Set 출력
        Set<String> keySet = strMap1.keySet();
        System.out.println(keySet);

        // Value Set 출력 -> valueSet(); 없음
